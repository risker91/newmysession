...
class Controller_Ajax extends Controller {
           ...
           private function goLog1($action, $group_id)
       {
           $up = new Model_Update();
           $up->saveAction($action,$group_id);  
         ...
         public function action_GetAns()
	{
            ...
            {
                     $buf_items .= '<div class="well"><p  id="'.$mas->id.'">'.$mas->desc.'...<a href="/session/answer/'.$mas->id.'">=>
                         <p style=""></a><a class="vote" id='.$mas->id.' vote="1"  ><img width="50" src="/assets/graph/like.jpg"></a>(12) <a class="vote" id='.$mas->id.' vote="-1"   ><img width="50" src="/assets/graph/dislike.png"></a>(1)</p></p></div>';
                                
           }
            echo json_encode(array("select"=>$buf_items));
         ...
         public function action_GetQuests()
	{
             ...
             $item = new Model_Item();         
             $quest = $item->show_questions($item_id);
             $data=null;
             
                foreach ($quest as $i=>$mas )
                {                     
                         $data[$i]['id']=$mas->id;
                         $data[$i]['text']=$mas->question;                
                }           
           echo json_encode($data);
